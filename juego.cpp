...
bool cargar(tJuego& juego, string nombre) {
	...
	if (!archivo.is_open()) {
		...
		juego.estado = JUGANDO;//Inicializo estado a jugando
		cargar(juego.tablero, archivo);//Carga tablero
		archivo.close();
	}
	return carga;
}
...
void ejecutarMovimiento(tJuego& juego) {//Hace un movimiento
	tMovimiento mov;
	int fila, col;
	mov = leeMovimiento(juego.tablero, fila, col);
